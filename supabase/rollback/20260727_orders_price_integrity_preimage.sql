-- ============================================================================
-- ROLLBACK PRE-IMAGE for 20260727_orders_price_integrity.sql
--
-- This is the VERBATIM body of public.lozi_orders_enforce_delivery_fee() as it
-- was deployed live (project niloddwnllhsvrmuxfxw) immediately BEFORE the price-
-- integrity change. Re-running this file restores the exact prior behavior
-- (delivery-fee/total enforcement only; NO item-price validation).
--
-- To roll back: run this file. It is a single CREATE OR REPLACE on one function;
-- the trigger binding (trg_orders_enforce_delivery_fee) is unchanged, so nothing
-- else needs to be touched. No data is modified (INSERT-time logic only).
-- ============================================================================
...
begin
  ...
  -- Non-admin UPDATE: pin the fee to whatever is already stored (sellers can never
  -- move it; any prior admin override is preserved).
  ...
  -- Seller resolution mirrors sync_order_seller_groups().
  with items as (
    select
      coalesce(
